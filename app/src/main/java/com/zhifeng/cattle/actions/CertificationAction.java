...
public class CertificationAction extends BaseAction<CertificationView> {
    public CertificationAction(RxAppCompatActivity _rxAppCompatActivity,CertificationView view) {
    ...
    public void certification(CertificationPost certificationPost){
        post(WebUrlUtil.POST_UPDATA_IDCARD_PIC,false,service -> manager.runHttp(
                service.PostData(CollectionsUtils.generateMap("token", MySp.getAccessToken(MyApp.getContext()),"name",certificationPost.getName(),
                        ...
                        WebUrlUtil.POST_UPDATA_IDCARD_PIC)
        ));
    }
        ...
        Observable.just(action.getErrorType())
            ...
            public void accept(Boolean aBoolean) throws Exception {
                ...
                switch (action.getIdentifying()) {
                    case WebUrlUtil.POST_UPDATA_IDCARD_PIC:
                        //todo 身份认证
                        if (aBoolean) {
