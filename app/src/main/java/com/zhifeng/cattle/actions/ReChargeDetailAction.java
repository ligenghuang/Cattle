...
public class ReChargeDetailAction extends BaseAction<ReChargeDetailView> {
    public ReChargeDetailAction(RxAppCompatActivity _rxAppCompatActivity, ReChargeDetailView reChargeDetailView) {
        ...
        Observable.just(action.getErrorType())
                .all(integer -> (integer == 200)).subscribe(aBoolean -> {
                    ...
                    switch (action.getIdentifying()) {
                        case WebUrlUtil.POST_RechargeDetail_LIST:
                            //todo 获取充值明细
                            if (aBoolean) {
                                L.e("xx", "输出返回结果 " + action.getUserData().toString());
                                ReChargeDetail reChargeDetail = new Gson().fromJson(action.getUserData().toString(), new TypeToken<ReChargeDetail>() {
                                }.getType());
                                if (reChargeDetail.getStatus() == 200){
                                    //todo 获取充值明细 成功
                                    view.getChargeDetailSuccess(reChargeDetail);
                                    return;
                                }
                                view.onError(reChargeDetail.getMsg(),action.getErrorType());
                                return;
                            }
