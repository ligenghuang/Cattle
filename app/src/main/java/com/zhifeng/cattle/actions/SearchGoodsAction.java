...
public class SearchGoodsAction extends BaseAction<SearchGoodsView> {
    ...
    public void getGoods(String keyword, int page, int num) {
        post(WebUrlUtil.POST_SEARCH_GOODS, false, service -> manager.runHttp(service.PostData(CollectionsUtils.generateMap("token", MySp.getAccessToken(MyApp.getContext()), "keyword", keyword, "page", page, "num", num), WebUrlUtil.POST_SEARCH_GOODS)));
    }
            ...
            switch (action.getIdentifying()) {
                case WebUrlUtil.POST_SEARCH_GOODS:
                    ...
                    if (aBoolean) {
                        ...
                        SearchGoods searchGoods = new Gson().fromJson(action.getUserData().toString(), new TypeToken<SearchGoods>() {
                        }.getType());
                        if (searchGoods.getStatus() == 1) {
                            //todo 获取搜索商品
                            view.getGoodsSuccess(searchGoods);
                            return;
                        }
                        view.onError(searchGoods.getMsg(), action.getErrorType());
                        return;
                    }
